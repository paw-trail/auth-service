...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http,
        ...
        http
                ...
                //
                // 이 빈은 공통 모듈이 만들어 준 것을 그대로 주입받음
                // 체인만 @ConditionalOnMissingBean(SecurityFilterChain.class) 이고
                // 핸들러 빈은 별도 조건이라 우리가 체인을 정의해도 살아 있음
                ...
                //
                // 이 서비스가 토큰을 발급하는 쪽이라고 해서 자기 요청의 토큰을 직접 파싱하지 않음
                ...
                .authorizeHttpRequests(auth -> auth
                        // 토큰을 받기 전에 불러야 하는 경로임
                        //
                        // 게이트웨이의 app.gateway.permit-all 과 짝이어야 함
                        // 게이트웨이는 이 경로들에 인증 헤더를 넣지 않고 통과시키므로
                        // 여기서 열어 두지 않으면 로그인과 회원가입이 401 로 막힙니다.
                        //
                        // 목록을 config 저장소에서 읽는 이유는 게이트웨이가 이미 그렇게 하고 있어서임
                        // 한쪽만 코드에 박아 두면 경로를 고칠 때 한쪽을 빠뜨리기 쉬움
                        .requestMatchers(permitAllPaths).permitAll()

                        // 아래 둘은 공통 체인과 같음
                        .requestMatchers("/internal/**", "/actuator/**").permitAll()

                        // 관리자 경로는 게이트웨이도 막고 여기서도 막음
                        //
                        // 겹치는 것이 낭비로 보이지만, 공통 체인이 물러나는 서비스가 바로 이 서비스라
                        // 여기에 없으면 이 서비스의 관리자 경로만 공통 보호를 잃음
                        // 게이트웨이가 막고 있어 뚫리지는 않으나 이중화가 반쪽이 됩니다.
                        //
                        // HeaderAuthenticationFilter 가 권한에 ROLE_ 을 붙이므로 hasRole 이 그대로 맞음
                        .requestMatchers("/api/v1/admin/**").hasRole("ADMIN")

                        // 나머지는 인증이 필요함
                        //
                        // 전부 열어 두지 않는 이유는 방어보다 불변조건 확보에 가까움
                        // 열어 두면 principal 이 null 인 채로 컨트롤러에 들어와
                        // 모든 자리에서 null 을 확인해야 함
