...
public class AuthService {
    ...
    public AccountResponse signup(String email, String rawPassword, String nickname) {
        ...
        // 이미 쓰이는 이메일인지 봅니다.
        //
        // 탈퇴한 계정은 여기에 걸리지 않습니다.
        // 탈퇴할 때 email 을 치환하므로 그 주소로 다시 가입할 수 있습니다.
        //
        // 한때 그것을 막아 두었던 이유는, 탈퇴 이벤트를 각 서비스가 소비하는 동안
        // 같은 주소로 새 계정이 생기면 방금 만든 계정의 데이터가 지워질 수 있다는 것이었습니다.
        // 그 우려는 성립하지 않습니다. 이벤트가 나르는 값이 계정 식별자 하나뿐이고
        // 받는 쪽이 전부 그 값으로 지우므로, 새 식별자를 받은 계정은 대상이 될 수 없습니다.
