...
public enum AuthErrorCode implements ErrorCode {
    ...
    // 짧은 시간에 메일을 너무 많이 요청함
    //
    // * 두 발송 경로 모두 인증이 필요 없어 누구나 계속 부를 수 있음
    //   막지 않으면 남의 주소로 메일이 쏟아지고,
    //   하루 발송 한도가 소진되어 정상 사용자도 메일을 못 받게 됨
    //
    // * 회원가입 인증에서만 이 코드를 내보냄
    //   비밀번호 재설정은 제한에 걸려도 성공으로 응답하고 발송만 건너뜀
    //   거기서 이 코드가 나가면 "이 이메일은 요청된 적이 있다" 가 드러남
    MAIL_SEND_COOLDOWN(HttpStatus.TOO_MANY_REQUESTS, "메일 요청이 너무 잦습니다. 잠시 후 다시 시도해 주세요."),
    ...
    //   아무 비밀번호나 넣고 응답만 비교해도 그 이메일이 가입되어 있음을 알 수 있음
    //   위 LOGIN_FAILED 로 계정 존재를 숨긴 것이 그 자리에서 무너집니다
    //
    // * 변경은 이미 로그인한 사람이 부르므로 숨길 것이 없음
    //   자기 계정이 소셜이라는 것은 본인이 아는 사실임
    //
    // * 재설정은 계정 존재를 숨겨야 하므로 이 코드를 내보내지 않음
    //   대상이 아니어도 항상 성공으로 응답함
